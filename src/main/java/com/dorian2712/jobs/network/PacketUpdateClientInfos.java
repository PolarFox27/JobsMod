...
public class PacketUpdateClientInfos implements IMessage {
    ...
    public Map<Entities, long[]> KILL_ENTITY_XP = new HashMap<>();
    public Map<Entities, Job> KILL_ENTITY_JOB = new HashMap<>();
    ...
    public PacketUpdateClientInfos(Map<Block, long[]> m1, Map<Block, Job> m2, Map<Item, long[]> m3, Map<Item, Job> m4,
                                   ...
                                   Map<Entities, long[]> m9, Map<Entities, Job> m10,
    ...
    public void fromBytes(ByteBuf buf)
    {
        ...
        {
            Entities e = Constants.Entities.byID(buf.readInt());
            Job job = Job.byIndex(buf.readInt());
            long[] xps = new long[25];
            for(int j = 0; j < 25; j++)
                xps[j] = buf.readLong();
            this.KILL_ENTITY_JOB.put(e, job);
            this.KILL_ENTITY_XP.put(e, xps);
        }
    ...
    public void toBytes(ByteBuf buf)
    {
        ...
        for(Entities e : this.KILL_ENTITY_JOB.keySet())//KILL ENTITY
        {
            buf.writeInt(e.id);
            buf.writeInt(this.KILL_ENTITY_JOB.get(e).index);
            for(int i = 0; i < 25; i++)
                buf.writeLong(this.KILL_ENTITY_XP.get(e)[i]);
        }
